...
5. Use MCP/tools only when declared in profile.
6. Run verify commands before DONE.
7. For unclear tasks, use `/discuss` first; do not implement while requirements are unresolved.
8. For external reference repos, use `company-reference-repo` and read targeted files only.
...
Do not commit OAuth tokens, API keys, `.env`, `auth.json`, or session files.
