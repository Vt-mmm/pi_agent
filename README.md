...
- Raw path-like tool access to protected paths is blocked before execution. This covers Pi built-ins such as `read`, `write`, `edit`, `grep`, `find`, `ls`, and custom/MCP tools when they expose common path fields like `path`, `filePath`, `targetPath`, or `file`.
- `grep.glob` and `find.pattern` are checked for patterns that can target protected paths. Broad `grep` sweeps also get a result-filter backstop that redacts returned lines from protected files.
