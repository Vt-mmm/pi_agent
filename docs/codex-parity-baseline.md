...
`v0.3.7` không cố copy OpenAI Codex. Mục tiêu là học các runtime invariant tốt của Codex và triển khai clean-room vào Pi package để dùng được cho solo/internal/team:
...
Trạng thái sau `v0.3.7`: **P3-baseline**. Tức là platform đã có module runtime để kiểm soát task tốt hơn P2, nhưng số liệu Pi vs Codex vs Claude vẫn phải chạy theo project/model thật trước khi tuyên bố token/cost reduction.
