...
Quan trọng: daily flow không bắt anh phải nhớ các lệnh này. Từ `v0.3.15`, các workflow `/task`, `/be-to-fe`, `/platform-improve`, `/plan`, `/review` có policy để parent agent tự cân nhắc spawn subagent khi task có phần việc độc lập. Slash command dưới đây dùng khi anh muốn ép orchestration cụ thể hoặc debug.
...
| `pi install git:github.com/Vt-mmm/pi_agent@v0.3.15` | Cài package platform từ GitHub. |
