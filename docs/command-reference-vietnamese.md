...
| `/model-options` | Giải thích model | Khi chưa rõ chọn Codex/Claude model nào, thinking nào. | Giải thích selector, scope, thinking, benchmark rule. |
...
| `/platform-migration` | Migration framework/agent docs | Khi nghiên cứu Pi docs, Codex CLI, Claude/Codex harness, repo tham khảo rồi áp dụng vào platform/project. | Có plan/source changes/docs/verify theo task migration. |
...
| `/model` | Chọn model | Muốn chọn OpenAI Codex/Claude bằng native selector. | Đây là flow chính, không phải hỏi agent tự chọn thay. |
| `Ctrl+L` | Mở model selector | Đổi model nhanh. | Tương đương UI selector của Pi. |
| `/scoped-models` | Chỉnh danh sách model cycle | Muốn `Ctrl+P` chỉ xoay quanh vài model hay dùng. | Global setup seed sẵn Codex + Claude. |
| `Ctrl+P` | Cycle model | Đổi model trong scope nhanh. | Dùng sau khi đã setup `enabledModels`. |
| `Shift+Ctrl+P` | Cycle model ngược | Quay lại model trước trong scope. | Tiện khi test Codex/Claude. |
...
Quan trọng: daily flow không bắt anh phải nhớ các lệnh này. Từ `v0.3.10`, các workflow `/task`, `/be-to-fe`, `/platform-migration`, `/plan`, `/review` có policy để parent agent tự cân nhắc spawn subagent khi task có phần việc độc lập. Slash command dưới đây dùng khi anh muốn ép orchestration cụ thể hoặc debug.
...
| `/review-loop` | Worker/reviewer/fix loop | Khi muốn review đến khi sạch hoặc hết vòng. | Nên set max rounds, mặc định upstream là 3. |
...
| `/parallel-handoff-plan` | Research + context + plan | Khi muốn handoff plan đầy đủ cho implementation. | Tốt cho migration/architecture lớn. |
...
Theo Codex/Claude docs, subagent tốt nhất cho việc độc lập và bounded:
...
### Migration Pi docs + Codex CLI/reference repo

```text
/platform-migration Research Pi docs, Codex CLI GitHub, Claude/Codex subagent docs, and mitsuhiko/agent-stuff. Apply only clean-room patterns that improve this Pi platform. Update docs, verify, and summarize sources.
```
...
Use company-scout to inspect relevant reference docs/repo patterns read-only.
Then use company-planner to produce a migration plan.
...
| `pi install git:github.com/Vt-mmm/pi_agent@v0.3.10` | Cài package platform từ GitHub. |
...
| `pi-company-model-scope --preset full` | Re-apply model scope Codex + Claude. |
...
| `pi-company-benchmark ...` | Ghi benchmark Pi/Codex/Claude bằng package bin. |
...
| `bash scripts/parity-benchmark.sh ...` | Ghi benchmark Pi vs Codex/Claude theo scenario thật. |
...
| `npm run benchmark -- ...` | `bash scripts/parity-benchmark.sh ...` |
...
Không claim “Pi tiết kiệm token hơn Codex/Claude CLI” nếu chưa có số liệu cùng scenario. Dùng benchmark script để ghi lại.
