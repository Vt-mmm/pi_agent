...
```bash
npm install -g @earendil-works/pi-coding-agent@0.80.10
pi install npm:@company/pi-agent-platform@0.4.2
```
...
```bash
bash /path/to/pi_agent/scripts/setup.sh /path/to/project --project-only --profile backend-api --package-source git:github.com/Vt-mmm/pi_agent@v0.4.2 --mcp-preset core --subagents-preset safe
```
