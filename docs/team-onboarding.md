...
```bash
npm install -g @earendil-works/pi-coding-agent
pi install npm:@company/pi_agent@0.1.0
```
...
```bash
bash /path/to/pi_agent/scripts/setup.sh /path/to/project --project-only --profile backend-api --package-source git:github.com/Vt-mmm/pi_agent@v0.1.0
```
