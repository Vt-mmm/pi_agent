...
Khi seed `.pi/settings.json` cho team/repo cần audit lặp lại, giữ package source dạng pinned tag như `git:github.com/Vt-mmm/pi_agent@v0.4.7`. Máy cá nhân có thể dùng `git:github.com/Vt-mmm/pi_agent` để theo latest.
...
Rollback hoặc pin version cụ thể:
...
| Command | Dùng để |
|---|---|
| `pi install git:github.com/Vt-mmm/pi_agent@v0.4.7` | Install pinned release cho reproducible team setup. |
...
| `pi-company-install --stable` | Apply stable release hiện tại. |
| `pi-company-install --version vX.Y.Z` | Install hoặc rollback về exact release tag. |
| `pi-company-install --dev` | Dùng moving source cho máy cá nhân/sandbox. |
| `pi update --extensions` | Update packages đã cài. |
