...
Runtime nên cài latest cho máy cá nhân; version cụ thể được lấy từ package metadata/release tag khi cần pin cho team.

```bash
pi install git:github.com/Vt-mmm/pi_agent
```

Lệnh trên cài global latest. Với repo/team cần reproducible lock, dùng pinned tag `git:github.com/Vt-mmm/pi_agent@vX.Y.Z` trong `.pi/settings.json`.
...
| Command | Dùng để |
|---|---|
| `pi install git:github.com/Vt-mmm/pi_agent` | Install global latest platform package. |
| `pi install git:github.com/Vt-mmm/pi_agent@vX.Y.Z` | Install pinned release cho reproducible setup. |
